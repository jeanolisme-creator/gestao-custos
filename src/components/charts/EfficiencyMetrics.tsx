...
export function EfficiencyMetrics({ data }: EfficiencyMetricsProps) {
  ...
  const schools = data.filter(school => school.totalValue > 0 && school.totalConsumption > 0).slice(0, 10);
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>Análise de eficiência de consumo</CardDescription>
        </CardHeader>
        <CardContent>
          <p className="text-center text-muted-foreground">Sem dados disponíveis para análise de eficiência</p>
        </CardContent>
      </Card>
    );
  }
  ...
  const avgEfficiency = totalValue > 0 ? (totalConsumption / totalValue) * 1000 : 0;
  ...
  const metrics = [
    {
      ...
      value: `${avgEfficiency.toFixed(2)} ${getUnit()}/R$1k`,
      progress: 75,
      icon: TrendingUp,
    ...
    },
    {
      ...
      progress: 65,
      icon: TrendingDown,
      description: "Mudança no último mês",
      color: "warning"
    }
  ];
    ...
    <Card className="p-6 bg-gradient-card border-border shadow-card">
      <div className="mb-6">
        ...
        <p className="text-sm text-muted-foreground">
          Análise de performance e otimização de recursos
        </p>
      </div>
          ...
          <div key={index} className="p-4 rounded-lg bg-background/50 border border-border">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center space-x-2">
                ...
                <span className="font-medium text-foreground text-sm">{metric.title}</span>
              </div>
              ...
              <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-success/5 border border-success/20">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {(school.totalConsumption || 0).toFixed(1)}{getUnit()} • {formatCurrency(school.totalValue || 0)}
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-success">
                    R$ {(school.costPerM3 || 0).toFixed(2)}/{getUnit()}
                  </p>
                </div>
              </div>
            ))}
      ...
      <div className="mt-6 grid grid-cols-2 md:grid-cols-3 gap-4">
        <div className="text-center p-3 rounded-lg bg-background/30">
          ...
          <p className="text-sm font-semibold text-foreground">
            {totalConsumption.toFixed(0)}{getUnit()}
          </p>
        </div>
        <div className="text-center p-3 rounded-lg bg-background/30">
