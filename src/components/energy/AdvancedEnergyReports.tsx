...
const availableFields = [
  { value: 'cadastro_cliente', label: 'Cadastro' },
  { value: 'relogio', label: 'Relógio' },
  { value: 'consumo_kwh', label: 'Consumo (KWh)' },
  { value: 'demanda_kwh', label: 'Demanda (KWh)' },
            ...
            {selectedFields.map((field) => (
              <TableCell key={field}>
                {field === 'valor_gasto' || field === 'valor_servicos'
                  ...
                  : field === 'consumo_kwh' || field === 'demanda_kwh'
                  ? `${(record[field] || 0).toFixed(1)} KWh`
                  : record[field] || '-'}
              </TableCell>
            ))}
          </TableRow>
        ...
        <TableRow>
          ...
          {selectedMonths.map((month) => (
            <TableHead key={month} className="text-center">
              {month}
              <div className="text-xs text-muted-foreground font-normal">KWh / Valor</div>
            </TableHead>
          ))}
        </TableRow>
      </TableHeader>
            ...
            {selectedMonths.map((month) => (
              <TableCell key={month} className="text-center">
                <div className="flex flex-col gap-1">
                  <span className="text-sm">{school[month]?.consumption.toFixed(1) || 0} KWh</span>
    ...
    <Table>
      <TableHeader>
        <TableRow>
          ...
          <TableHead className="text-right">KWh/Aluno</TableHead>
      ...
      <TableBody>
        {comparisonData.map((school, index) => (
          <TableRow key={index}>
            ...
            <TableCell className="text-right">{school.totalConsumption.toFixed(1)} KWh</TableCell>
    ...
    <Table>
      ...
      <TableBody>
        {comparisonData.map((item, index) => (
          <TableRow key={index}>
            ...
            <TableCell className="text-right">{item.totalConsumption.toFixed(1)} KWh</TableCell>
            <TableCell className="text-right font-semibold">{formatCurrency(item.totalValue)}</TableCell>
            <TableCell className="text-right">{item.avgConsumption.toFixed(1)} KWh</TableCell>
            <TableCell className="text-right text-primary font-semibold">
      ...
      <TableFooter>
        <TableRow>
          ...
          <TableCell className="text-right font-bold">{totals.totalConsumption.toFixed(1)} KWh</TableCell>
